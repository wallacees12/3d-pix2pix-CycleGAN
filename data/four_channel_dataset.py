...
class FourChannelDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        mr_data = np.load(mr_path)['data']  # Shape: [D, H, W, C] or [D, H, W]
        
        # Load CT data (1-channel)
        ct_path = os.path.join(self.root, sample['ct_path'])
        if not os.path.exists(ct_path):
            raise FileNotFoundError(f"CT file not found: {ct_path}")
        
        ct_data = np.load(ct_path)['data']  # Shape: [D, H, W]
        ...
        return {
            ...
            'B': ct_tensor,           # 1-channel CT
            'A_paths': mr_path,       # MR file path
            'B_paths': ct_path        # CT file path
        }
